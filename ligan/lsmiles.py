...
def to_smiles(data: 'torch_geometric.data.Data',
    ...
    for i in range(data.num_nodes):
        ...
        atom.SetChiralTag(Chem.rdchem.ChiralType.values[0])#int(data.x[i, 1])])
        atom.SetFormalCharge(x_map['formal_charge'][int(data.x[i, 1])])
        atom.SetNumExplicitHs(x_map['num_hs'][int(data.x[i, 2])])
